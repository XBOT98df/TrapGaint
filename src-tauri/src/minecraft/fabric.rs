use serde::{Deserialize, Serialize};
use std::fs;

const FABRIC_META_URL: &str = "https://meta.fabricmc.net/v2";

#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct FabricVersionInfo {
    pub id: String,
    pub mc_version: String,
    pub loader_version: String,
    pub stable: bool,
}

#[derive(Debug, Deserialize)]
struct FabricLoaderVersion {
    separator: String,
    build: u32,
    maven: String,
    version: String,
    stable: bool,
}

#[derive(Debug, Deserialize)]
struct FabricGameVersion {
    version: String,
    stable: bool,
}

impl super::MinecraftLauncher {
    /// Get all available Fabric loader versions
    pub async fn get_fabric_versions(&self) -> Result<Vec<FabricVersionInfo>, String> {
        let client = reqwest::Client::builder()
            .user_agent("MinecraftLauncher/1.0")
            .build()
            .map_err(|e| e.to_string())?;

        // Get supported game versions
        let game_versions_url = format!("{}/versions/game", FABRIC_META_URL);
        let game_response = client
            .get(&game_versions_url)
            .send()
            .await
            .map_err(|e| format!("Failed to fetch Fabric game versions: {}", e))?;

        let game_versions: Vec<FabricGameVersion> = game_response
            .json()
            .await
            .map_err(|e| format!("Failed to parse Fabric game versions: {}", e))?;

        // Get loader versions
        let loader_url = format!("{}/versions/loader", FABRIC_META_URL);
        let loader_response = client
            .get(&loader_url)
            .send()
            .await
            .map_err(|e| format!("Failed to fetch Fabric loader versions: {}", e))?;

        let loader_versions: Vec<FabricLoaderVersion> = loader_response
            .json()
            .await
            .map_err(|e| format!("Failed to parse Fabric loader versions: {}", e))?;

        // Get the latest stable loader version
        let latest_loader = loader_versions
            .iter()
            .find(|l| l.stable)
            .or_else(|| loader_versions.first())
            .ok_or("No Fabric loader versions found")?;

        // Create version info for each supported game version with the latest loader
        let mut fabric_versions: Vec<FabricVersionInfo> = game_versions
            .iter()
            .filter(|g| g.stable) // Only stable game versions
            .map(|game| FabricVersionInfo {
                id: format!("fabric-loader-{}-{}", latest_loader.version, game.version),
                mc_version: game.version.clone(),
                loader_version: latest_loader.version.clone(),
                stable: latest_loader.stable,
            })
            .collect();

        // Sort by MC version (newest first)
        fabric_versions.sort_by(|a, b| version_compare(&b.mc_version, &a.mc_version));

        Ok(fabric_versions)
    }

    /// Get installed Fabric versions
    pub fn get_installed_fabric_versions(&self) -> Result<Vec<String>, String> {
        let versions_dir = self.game_dir.join("versions");
        let mut installed = Vec::new();

        if versions_dir.exists() {
            for entry in fs::read_dir(&versions_dir).map_err(|e| e.to_string())? {
                let entry = entry.map_err(|e| e.to_string())?;
                let name = entry.file_name().to_string_lossy().to_string();

                // Fabric versions start with "fabric-loader-"
                if name.starts_with("fabric-loader-") {
                    println!("[INFO] Found Fabric version: {}", name);
                    installed.push(name);
                }
            }
        }

        // Sort by version (newest first)
        installed.sort_by(|a, b| version_compare(b, a));

        Ok(installed)
    }

    /// Get Fabric versions available for a specific Minecraft version
    pub async fn get_fabric_versions_for_mc(
        &self,
        mc_version: &str,
    ) -> Result<Vec<FabricVersionInfo>, String> {
        let client = reqwest::Client::builder()
            .user_agent("MinecraftLauncher/1.0")
            .build()
            .map_err(|e| e.to_string())?;

        // Get loader versions for this specific MC version
        let url = format!("{}/versions/loader/{}", FABRIC_META_URL, mc_version);
        let response = client
            .get(&url)
            .send()
            .await
            .map_err(|e| format!("Failed to fetch Fabric loaders for {}: {}", mc_version, e))?;

        if !response.status().is_success() {
            return Ok(Vec::new()); // MC version not supported by Fabric
        }

        #[derive(Deserialize)]
        struct LoaderInfo {
            loader: FabricLoaderVersion,
        }

        let loaders: Vec<LoaderInfo> = response
            .json()
            .await
            .map_err(|e| format!("Failed to parse Fabric loaders: {}", e))?;

        let versions: Vec<FabricVersionInfo> = loaders
            .into_iter()
            .map(|l| FabricVersionInfo {
                id: format!("fabric-loader-{}-{}", l.loader.version, mc_version),
                mc_version: mc_version.to_string(),
                loader_version: l.loader.version,
                stable: l.loader.stable,
            })
            .collect();

        Ok(versions)
    }

    /// Install Fabric for a specific Minecraft version
    pub async fn install_fabric<F>(
        &self,
        fabric_version: &FabricVersionInfo,
        progress_callback: F,
    ) -> Result<String, String>
    where
        F: Fn(f32, String) + Send + Sync,
    {
        progress_callback(
            0.0,
            format!(
                "Installing Fabric {} for Minecraft {}...",
                fabric_version.loader_version, fabric_version.mc_version
            ),
        );

        // First ensure vanilla Minecraft is FULLY installed (JAR, JSON, and libraries)
        let vanilla_jar = self
            .versions_dir
            .join(&fabric_version.mc_version)
            .join(format!("{}.jar", fabric_version.mc_version));
        let vanilla_json = self
            .versions_dir
            .join(&fabric_version.mc_version)
            .join(format!("{}.json", fabric_version.mc_version));

        // Check if vanilla is fully installed (both JAR and JSON must exist)
        if !vanilla_jar.exists() || !vanilla_json.exists() {
            progress_callback(
                0.01,
                format!(
                    "Installing Minecraft {} first...",
                    fabric_version.mc_version
                ),
            );

            // Install vanilla Minecraft first
            self.install_version(&fabric_version.mc_version, |p, msg| {
                // Scale progress from 0.01 to 0.05 for vanilla installation
                let scaled_progress = 0.01 + (p * 0.04);
                progress_callback(scaled_progress, format!("[Vanilla] {}", msg));
            })
            .await
            .map_err(|e| {
                format!(
                    "Failed to install Minecraft {}: {}",
                    fabric_version.mc_version, e
                )
            })?;

            progress_callback(
                0.05,
                format!(
                    "Minecraft {} installed, continuing with Fabric...",
                    fabric_version.mc_version
                ),
            );
        }

        let client = reqwest::Client::builder()
            .user_agent("MinecraftLauncher/1.0")
            .timeout(std::time::Duration::from_secs(180)) // Increased timeout for 1.21.5+
            .connect_timeout(std::time::Duration::from_secs(30))
            .build()
            .map_err(|e| e.to_string())?;

        progress_callback(0.1, "Fetching Fabric profile...".to_string());

        // Fetch the version profile from Fabric Meta API
        let profile_url = format!(
            "{}/versions/loader/{}/{}/profile/json",
            FABRIC_META_URL, fabric_version.mc_version, fabric_version.loader_version
        );

        println!("Fetching Fabric profile from: {}", profile_url);

        let response = client
            .get(&profile_url)
            .send()
            .await
            .map_err(|e| format!("Failed to fetch Fabric profile: {}", e))?;

        if !response.status().is_success() {
            return Err(format!(
                "Failed to fetch Fabric profile: HTTP {}",
                response.status()
            ));
        }

        let profile_json: serde_json::Value = response
            .json()
            .await
            .map_err(|e| format!("Failed to parse Fabric profile: {}", e))?;

        // Get the version ID from the profile
        let version_id = profile_json["id"]
            .as_str()
            .ok_or("No version ID in Fabric profile")?
            .to_string();

        println!("Fabric version ID: {}", version_id);

        progress_callback(0.2, format!("Creating version {}...", version_id));

        // Create version directory
        let version_dir = self.versions_dir.join(&version_id);
        fs::create_dir_all(&version_dir)
            .map_err(|e| format!("Failed to create version directory: {}", e))?;

        // Write version JSON
        let json_path = version_dir.join(format!("{}.json", version_id));
        fs::write(
            &json_path,
            serde_json::to_string_pretty(&profile_json).unwrap(),
        )
        .map_err(|e| format!("Failed to write version JSON: {}", e))?;

        println!("Wrote version JSON to: {:?}", json_path);

        progress_callback(
            0.3,
            "Downloading Fabric libraries (parallel)...".to_string(),
        );

        // Download all required libraries IN PARALLEL for speed
        if let Some(libraries) = profile_json["libraries"].as_array() {
            use super::downloader::{download_parallel, DownloadTask};

            let mut tasks: Vec<DownloadTask> = Vec::new();

            for lib in libraries.iter() {
                // Get library info
                if let Some(name) = lib["name"].as_str() {
                    // Parse Maven coordinates: group:artifact:version[:classifier]
                    let parts: Vec<&str> = name.split(':').collect();
                    if parts.len() >= 3 {
                        let group = parts[0].replace('.', "/");
                        let artifact = parts[1];
                        let version = parts[2];
                        let classifier = parts.get(3).map(|s| *s);

                        let jar_name = if let Some(cls) = classifier {
                            format!("{}-{}-{}.jar", artifact, version, cls)
                        } else {
                            format!("{}-{}.jar", artifact, version)
                        };

                        let lib_path = self
                            .libraries_dir
                            .join(&group)
                            .join(artifact)
                            .join(version)
                            .join(&jar_name);

                        // Skip if already exists
                        if lib_path.exists() {
                            continue;
                        }

                        // Create parent directory
                        if let Some(parent) = lib_path.parent() {
                            fs::create_dir_all(parent).ok();
                        }

                        // Get download URL - Fabric profile has "url" at library level as base URL
                        let base_url = lib["url"].as_str().unwrap_or("https://maven.fabricmc.net/");
                        let url = format!(
                            "{}{}/{}/{}/{}",
                            base_url, group, artifact, version, jar_name
                        );

                        // Add fallback URLs
                        let fallback_urls = vec![
                            format!(
                                "https://maven.fabricmc.net/{}/{}/{}/{}",
                                group, artifact, version, jar_name
                            ),
                            format!(
                                "https://libraries.minecraft.net/{}/{}/{}/{}",
                                group, artifact, version, jar_name
                            ),
                            format!(
                                "https://repo1.maven.org/maven2/{}/{}/{}/{}",
                                group, artifact, version, jar_name
                            ),
                        ];

                        tasks.push(DownloadTask {
                            url,
                            path: lib_path,
                            fallback_urls,
                            expected_sha1: None,
                        });
                    }
                }
            }

            // Download all libraries in parallel (12 concurrent for better stability on 1.21.5+)
            // Use timeout wrapper to prevent hanging
            println!("[Fabric] Downloading {} libraries...", tasks.len());

            let download_future = download_parallel(
                tasks,
                12, // Reduced from 16 for better stability
                |progress, status| {
                    let overall_progress = 0.3 + (0.6 * progress);
                    progress_callback(overall_progress, status);
                },
                0.0,
                1.0,
                "Downloading Fabric libraries",
            );

            // Add timeout to prevent hanging (5 minutes max)
            match tokio::time::timeout(std::time::Duration::from_secs(300), download_future).await {
                Ok((downloaded, failed)) => {
                    println!(
                        "Fabric libraries: {} downloaded, {} failed",
                        downloaded, failed
                    );

                    // If no libraries were downloaded at all, that's a critical error
                    if downloaded == 0 && failed > 0 {
                        println!("[ERROR] All Fabric library downloads failed");
                        return Err(format!("Failed to download Fabric libraries ({} failures). Please check your internet connection.", failed));
                    }

                    // If too many failures, warn but continue (some libraries might be optional)
                    if failed > 0 && failed < downloaded / 2 {
                        println!(
                            "[WARN] Some Fabric libraries failed to download, but continuing..."
                        );
                    } else if failed >= downloaded / 2 && downloaded > 0 {
                        return Err(format!(
                            "Too many library download failures: {}/{}",
                            failed,
                            downloaded + failed
                        ));
                    }
                }
                Err(_) => {
                    println!("[ERROR] Fabric library download timed out after 5 minutes");
                    return Err("Library download timed out. Please check your internet connection and try again.".to_string());
                }
            }
        }

        progress_callback(0.95, "Finalizing installation...".to_string());

        // Verify installation
        let json_exists = json_path.exists();

        if !json_exists {
            return Err("Fabric installation failed: version JSON not created".to_string());
        }

        // Create profile in state system (like Modrinth)
        let profile = super::state::Profile {
            id: version_id.clone(),
            name: format!("Fabric {}", fabric_version.mc_version),
            game_version: fabric_version.mc_version.clone(),
            loader: "fabric".to_string(),
            loader_version: Some(fabric_version.loader_version.clone()),
            install_stage: super::state::InstallStage::Installed,
            java_path: None,
            java_version: None,
            memory_mb: None,
            created: chrono::Utc::now(),
            modified: chrono::Utc::now(),
            last_played: None,
            icon_url: None,
            project_id: None,
            version_id: None,
            mod_count: None,
        };

        self.state.upsert_profile(&profile).ok();
        println!("[Fabric] Created profile in state: {}", version_id);

        progress_callback(
            1.0,
            format!("Fabric {} installed successfully!", version_id),
        );

        Ok(version_id)
    }
}

/// Compare version strings (e.g., "1.20.1" vs "1.19.4")
fn version_compare(a: &str, b: &str) -> std::cmp::Ordering {
    let parse_version = |s: &str| -> Vec<u32> {
        s.split(|c: char| !c.is_ascii_digit())
            .filter_map(|part| part.parse::<u32>().ok())
            .collect()
    };

    let a_parts = parse_version(a);
    let b_parts = parse_version(b);

    for (a_part, b_part) in a_parts.iter().zip(b_parts.iter()) {
        match a_part.cmp(b_part) {
            std::cmp::Ordering::Equal => continue,
            other => return other,
        }
    }

    a_parts.len().cmp(&b_parts.len())
}
