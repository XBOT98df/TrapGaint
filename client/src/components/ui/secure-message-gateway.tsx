    ...
    <div className="absolute z-10 inset-0 pointer-events-none">
      {positions.map((pos, index) => (
        <section key={index} className={`absolute ${pos}`}>
          <svg className="w-4 h-4 opacity-70" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg">
            <g fill="none" fillRule="evenodd">
              <g fill="currentColor" className="text-green-400">
      ...
      <div className="relative w-full max-w-2xl bg-transparent border border-border border-dashed shadow-sm p-6 sm:p-10 transition-all rounded-none">
        ...
        <div className="min-h-full z-0 w-full bg-transparent absolute top-0 left-0 pointer-events-none">
          ...
          <div
            ...
            style={{
              backgroundImage: `linear-gradient(to right, var(--muted) 1px, transparent 1px),
        ...
        <div className="backdrop-blur-xs p-2 rounded-xs">
          ...
          <div className="mb-6 z-10">
            ...
            <h1 className="text-xl font-semibold text-foreground tracking-tight">
              <span className="text-green-400">Contact </span>with 21st
            </h1>
          </div>
        ...
        <div className="mt-6 flex items-center justify-between">
          <span className="text-[0.55rem] font-mono uppercase tracking-widest text-muted-foreground">
            Status:{" "}
            {pending ? (
              <span className="text-muted-foreground">Establishing Handshake...</span>
            ) : (
              <span className="text-green-500 border p-0.5 border-green-500 ">Ready</span>
            )}
          </span>
          <span className="text-[0.55rem] font-mono text-muted-foreground/40">SECURE_ID: 0x44FE1</span>
        </div>
      </div>
    </div>
  );
